...
if (isset($_POST['send'])) {
    ...
    $editnickname = mysqli_real_escape_string($conn, $_POST["editnickname"]);

    $edittypes = mysqli_real_escape_string($conn, $_POST["edittypes"]);

    $user_id = $_GET["user_edit"];

    $ses_id = $_SESSION["u_id"];

    if ($user_id and $_SESSION["status"] > 1 and $editpass !== "") {

        $u_select = "SELECT * FROM user WHERE id = '$user_id'";

        $res = mysqli_query($conn, $u_select);

        if (mysqli_num_rows($res) > 0) {

            $sql = "UPDATE user  SET names = '{$editname}', nickname = '{$editnickname}', u_status = '{$edittypes}' WHERE id='$user_id'";
                ...
                <div class="formReg">
                    ...
                    <div class="regMid">
                        ...
                        <form action="" method="post" enctype="multipart/form-data">
                            ...
                            $us_select = "SELECT * FROM user WHERE id = '$user_id'";
                            ...
                            if (mysqli_num_rows($res) > 0) {
                                ...
                                while ($row = mysqli_fetch_array($res)) {
                                    ...
                                    echo "<input class='regInput' type='text' name='editname' value='" . $row['names'] . "' placeholder=" . $row['names'] . ">";

                                    echo "<input class='regInput' type='text' name='editnickname'value='" . $row['nickname'] . "' placeholder=" . $row['nickname'] . ">";
                                    ...
                                    <select class='form-select regInput' name='edittypes' aria-label='Default select example'>

                                        <option " . $a . " value='1'>Personel</option>

                                        <option " . $b . " value='2'>Yönetici</option>
